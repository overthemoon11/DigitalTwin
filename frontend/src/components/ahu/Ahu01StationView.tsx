...
function ExternalDamperBox({ x, y, w, h, label, selected, onSelect, flowDir, loop }: {
  ...
  return (
    <g className="plant-equip" onClick={onSelect} style={{ cursor: 'pointer' }}>
      ...
      <text x={x + w / 2} y={y + h / 2 + 4} textAnchor="middle" fill={SCADA.tag} fontSize={8} fontWeight="700">{label}</text>
...
function AhuFan({ x, y, fan, selected, onSelect }: {
  x: number; y: number; fan: AhuState['saFan']; selected: boolean; onSelect: () => void;
}) {
  ...
  return (
    <g className="plant-equip" onClick={onSelect} style={{ cursor: 'pointer' }}>
      ...
      <g transform={`translate(${x}, ${y})`}>
        ...
        <polygon points="-6,-5 6,-5 0,6" fill={col} opacity={0.85} />
      </g>
...
export default function Ahu01StationView({ state, selectedId, onSelect }: Props) {
  ...
  const faPct = damper('ahu01-oa-damper')?.positionPct ?? h.oaFraction * 100;
  const eaPct = damper('ahu01-ea-damper')?.positionPct ?? 15;
  ...
  return (
    <div className="chiller-plant-2d ahu-station-2d">
      ...
      >
        ...
        <ExternalDamperBox {...EXT_EA} label="EA DAMPER" flowDir="left" loop="ra"
          selected={selectedId === 'ahu01-ea-damper'} onSelect={() => onSelect('ahu01-ea-damper')} />
        <ExternalDamperBox {...EXT_FA} label="FA DAMPER" flowDir="right" loop="sa"
          selected={selectedId === 'ahu01-oa-damper'} onSelect={() => onSelect('ahu01-oa-damper')} />
        ...
        <AhuDamper x={POS.raEa.x} y={POS.raEa.y} label="EA DAMPER" pct={eaPct} open={eaPct > 10}
          selected={selectedId === 'ahu01-ea-damper'} onSelect={() => onSelect('ahu01-ea-damper')} />
        <AhuFan x={POS.raFan.x} y={POS.raFan.y} fan={state.raFan}
        ...
        {filter('ahu01-ra-eu7') && (
          <AhuFilter x={POS.raEu7.x} y={POS.raEu7.y} w={POS.raEu7.w} h={POS.raEu7.h}
            name="EU-7 FILTER" status={filter('ahu01-ra-eu7')!.status}
            selected={selectedId === 'ahu01-ra-eu7'} onSelect={() => onSelect('ahu01-ra-eu7')} />
        )}
        <AhuDamper x={POS.raFire.x} y={POS.raFire.y} label="FIRE DAMPER" pct={100} open
          selected={selectedId === 'ahu01-ra-fire'} onSelect={() => onSelect('ahu01-ra-fire')} />

        <SensorProbe x={POS.raSmoke.x} y={POS.raSmoke.y + 14} label="SMOKE SENSOR" />
        <StatusDot x={POS.raSmoke.x} y={POS.raSmoke.y + 2} ok />
        <FlowArrow x={POS.raSmoke.x + 10} y={POS.raSmoke.y + 14} dir="left" loop="ra" />
        <ScadaTag x={POS.raTrh.x - 36} y={POS.raTrh.y} tag="T & RH" pv={`${h.ratC}° / ${h.raRhPct}%`} width={96} />
        <SensorProbe x={POS.raCfm.x} y={POS.raCfm.y + 20} label="AIRFLOW" />
        <ScadaTag x={POS.raCfm.x - 40} y={POS.raCfm.y + 28} tag="RA CFM" pv={`${h.raCfm.toFixed(1)}`} unit="CFM" width={88} compact />
        ...
        <g className="plant-equip" onClick={() => onSelect('ahu01-ra-damper')} style={{ cursor: 'pointer' }}>
          ...
          <text x={RA_DAMPER.x + RA_DAMPER.w / 2} y={RA_DAMPER.y - 6} textAnchor="middle" fill={SCADA.tag} fontSize={8} fontWeight="700">RA DAMPER</text>
          <StatusDot x={RA_DAMPER.x + RA_DAMPER.w / 2} y={RA_DAMPER.y - 16} ok={raDamperPct > 20} />
        ...
        <AhuDamper x={POS.saFa.x} y={POS.saFa.y} label="FA DAMPER" pct={faPct} open={faPct > 5}
          selected={selectedId === 'ahu01-oa-damper'} onSelect={() => onSelect('ahu01-oa-damper')} />
        ...
        <AhuFan x={POS.saFan.x} y={POS.saFan.y} fan={state.saFan}
        ...
        {filter('ahu01-sa-eu7') && (
          <AhuFilter x={POS.saEu7.x} y={POS.saEu7.y} w={POS.saEu7.w} h={POS.saEu7.h}
            name="EU-7 FILTER" status={filter('ahu01-sa-eu7')!.status}
        ...
        <AhuDamper x={POS.saFire.x} y={POS.saFire.y} label="FIRE DAMPER" pct={100} open
          selected={selectedId === 'ahu01-sa-fire'} onSelect={() => onSelect('ahu01-sa-fire')} />

        <SensorProbe x={POS.saCfm.x} y={POS.saCfm.y - 8} label="AIRFLOW" />
        <ScadaTag x={POS.saCfm.x - 40} y={POS.saCfm.y} tag="SA CFM" pv={`${h.saCfm.toFixed(1)}`} unit="CFM" width={88} compact />
        <FlowArrow x={POS.saCfm.x + 44} y={POS.saCfm.y + 8} dir="right" loop="sa" />

        <text x={RA_BOX.x - 8} y={RA_CY + 4} textAnchor="end" fill={SCADA.tag} fontSize={9} fontWeight="700">EA</text>
        <FlowArrow x={RA_BOX.x - 12} y={RA_CY} dir="left" loop="ra" />
        <text x={SA_BOX.x - 8} y={SA_CY + 4} textAnchor="end" fill={SCADA.tag} fontSize={9} fontWeight="700">FA</text>
        ...
        <g className="plant-equip" onClick={() => onSelect('room')} style={{ cursor: 'pointer' }}>
          ...
          <FlowArrow x={ROOM.x + 20} y={ROOM.y + 8} dir="down" loop="sa" />
          <FlowArrow x={ROOM.x + 32} y={ROOM.y + 20} dir="down" loop="sa" />
          <FlowArrow x={ROOM.x + ROOM.w - 20} y={ROOM.y + 8} dir="up" loop="ra" />
          <FlowArrow x={ROOM.x + ROOM.w - 32} y={ROOM.y + 20} dir="up" loop="ra" />
        </g>
      </svg>
    </div>
  );
