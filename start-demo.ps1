...
$existingBackend = Get-NetTCPConnection -LocalPort 3005 -ErrorAction SilentlyContinue
$existingFrontend = Get-NetTCPConnection -LocalPort 3004 -ErrorAction SilentlyContinue

if ($existingBackend) {
    Write-Host "  Port 3005 in use - attempting to free..." -ForegroundColor Yellow
    Stop-Process -Id (Get-Process -Id $existingBackend.OwningProcess).Id -Force -ErrorAction SilentlyContinue
    Start-Sleep -Milliseconds 500
}
if ($existingFrontend) {
    Write-Host "  Port 3004 in use - attempting to free..." -ForegroundColor Yellow
...
Write-Host 'Starting server on http://localhost:3005...' -ForegroundColor Yellow
Write-Host ''
try {
    npm start
} catch {
...
"@
    ...
    while ($waited -lt $maxWait) {
        ...
        $waited++
        try {
            $response = Invoke-WebRequest -Uri "http://localhost:3005/api/twin" -TimeoutSec 2 -ErrorAction SilentlyContinue
            if ($response.StatusCode -eq 200) {
                Write-Host "  [OK] Backend running on http://localhost:3005" -ForegroundColor Green
                break
            }
        } catch {
...
Write-Host 'Starting Vite dev server on http://localhost:3004...' -ForegroundColor Yellow
Write-Host ''
npm run dev
Write-Host ''
Write-Host 'Press any key to close...' -ForegroundColor Gray
...
"@
    ...
    Write-Host "  [OK] Frontend starting on http://localhost:3004" -ForegroundColor Green
}
...
Write-Host "  Frontend UI:  http://localhost:3004" -ForegroundColor White
Write-Host "  WebSocket:    ws://localhost:3005/ws" -ForegroundColor White
Write-Host ""
