...
$existingBackend = Get-NetTCPConnection -LocalPort 3007 -ErrorAction SilentlyContinue
$existingFrontend = Get-NetTCPConnection -LocalPort 3006 -ErrorAction SilentlyContinue

if ($existingBackend) {
    Write-Host "  Port 3007 in use - attempting to free..." -ForegroundColor Yellow
    Stop-Process -Id (Get-Process -Id $existingBackend.OwningProcess).Id -Force -ErrorAction SilentlyContinue
    Start-Sleep -Milliseconds 500
}
if ($existingFrontend) {
    Write-Host "  Port 3006 in use - attempting to free..." -ForegroundColor Yellow
...
Write-Host 'Starting server on http://localhost:3007...' -ForegroundColor Yellow
Write-Host ''
try {
    npm start
} catch {
...
"@
    ...
    while ($waited -lt $maxWait) {
        ...
        $waited++
        try {
            $response = Invoke-WebRequest -Uri "http://localhost:3007/api/twin" -TimeoutSec 2 -ErrorAction SilentlyContinue
            if ($response.StatusCode -eq 200) {
                Write-Host "  [OK] Backend running on http://localhost:3007" -ForegroundColor Green
                break
            }
        } catch {
...
Write-Host 'Starting Vite dev server on http://localhost:3006...' -ForegroundColor Yellow
Write-Host ''
npm run dev
Write-Host ''
Write-Host 'Press any key to close...' -ForegroundColor Gray
...
"@
    ...
    Write-Host "  [OK] Frontend starting on http://localhost:3006" -ForegroundColor Green
}
...
Write-Host "  Frontend UI:  http://localhost:3006" -ForegroundColor White
Write-Host "  WebSocket:    ws://localhost:3007/ws" -ForegroundColor White
Write-Host ""
