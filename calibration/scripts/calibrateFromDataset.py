...
Emits backend/src/digital-twin/chiller/calibration/t1MonthCalibration.ts
(plus the t1MvRows / t1Row86 fixtures and the month-validation archive).

  python calibration/scripts/calibrateFromDataset.py [--refresh]
...
TWIN = REPO / "backend" / "src" / "digital-twin" / "chiller"
OUT_TS = TWIN / "calibration" / "t1MonthCalibration.ts"
OUT_MV_ROWS = TWIN / "fixtures" / "t1MvRows.ts"
OUT_ROW86 = TWIN / "fixtures" / "t1Row86.ts"
OUT_VALIDATION = REPO / "calibration" / "fixtures" / "t1-month-validation.json.gz"
...
def main() -> int:
    ...
    emit_mv_rows(mv)
    print(f"wrote {OUT_MV_ROWS.relative_to(REPO)}  ({len(mv)} rows)")

    # ---- row-86 per-point expectations ---------------------------------------
    emit_row86(D, 85, cols_for_points())
    print(f"wrote {OUT_ROW86.relative_to(REPO)}")
    return 0
 ...
 *
 * GENERATED by calibration/scripts/calibrateFromDataset.py — do not hand-edit.
 *
 ...
 * window as tightly as it used to. Use calibration/validation/validateMonth.ts for
...
'''
    OUT_MV_ROWS.write_text(ts, encoding="utf-8")
 ...
 * calibration/scripts/calibrateFromDataset.py; do not hand-edit.
 *
...
'''
    OUT_ROW86.write_text(ts, encoding="utf-8")
 ...
 * calibration/scripts/calibrateFromDataset.py from
